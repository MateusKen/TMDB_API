...
class ValidacaoNotFoundFilmeTest {

    @InjectMocks
    private ValidacaoNotFoundFilme validacao;
    ...
    @Test
    @DisplayName("Teste - Validação procura filme por id e acha no banco")
    void ValidacaoNotFoundFilmeNaoRetornaErro() {
    ...
    void ValidacaoNotFoundFilmeRetornaErro(){
        when(filmeRepository.findById(anyLong())).thenReturn(Optional.empty());
        assertThrows(NotFoundException.class, () -> validacao.validar(filme));
    }
